...
function check_file(f){
  ...
  if(type == "image/jpeg" || type== "image/png"|| type == "image/gif"){
    if (size > 10000)// checks the file more than 5 MB
    {
...
jQuery.validator.addMethod("uploadFile", function (val, element) {
  var files = element.files; //all files upload
  var length = files.length;
  for(var i=0; i < length; i++){
      console.log("file: " + i);
      console.log(files[i]);
      console.log("check file: " + i);
      console.log(check_file(files[i]));
      if(check_file(files[i])==false){
        // console.log("input");
  ...
  // $('#customFile')[0].files.splice(0,1);
  var newFileList = Array.from($('#customFile')[0].files);
  newFileList.splice(1,1);
  console.log(newFileList);
  console.log("final input");
  // console.log(window.files);
// $('#customFile')[0].files.pop();
  console.log($('#customFile')[0].files);
  Object.assign($('#customFile')[0].files, newFileList);
  console.log($('#customFile')[0].files);
